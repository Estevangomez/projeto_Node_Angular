...
export class LoginService {
	...
	logar(usuario: any, linha, familia) {
		...
		if (usuario.nome != undefined) {
			...
			sessionStorage.setItem('empresa', this.usuario.empresa);
			...
			sessionStorage.setItem('linha', `${this.linha}`);
			sessionStorage.setItem('familia', `${this.familia}`);
    ...
    passarInfo(){		
	 	let atv = window.sessionStorage.getItem('atividade')
        return atv;
