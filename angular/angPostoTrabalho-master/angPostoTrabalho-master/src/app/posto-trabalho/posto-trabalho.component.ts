...
export class PostoTrabalhoComponent implements OnInit {
	...
	mostrarinfo = new EventEmitter<boolean>();	


	constructor(private apiService: ApiService, private route: ActivatedRoute, private router: Router, private login:LoginService) {
			...
			if (this.postoVazio[0].codfun != null) {
				...
				this.apiService.getUsersDigitais().subscribe((dataD: any) => {
					...
					this.apiService.getUrlBiometria(param).subscribe(dataU => {
						...
						let resp_sessao = JSON.parse(sessionStorage.getItem('codfun'));				


						this.apiService.putAssociarColabPosto(idposto, resp_sessao, this.associarColab).subscribe((associarColab) => {
		...
		this.apiService.getPtConfig(produto, qtd).subscribe((dataPtConfig: any) => {
			...
			if (this.tipo != undefined || this.posto != null) {
				...
				this.apiService.getTipoPostoByTipo(atv, this.ptConfig[0].codigo, linha, this.tipo).subscribe((dataPostosTipos: any) => {
					...
					this.apiService.getTipoPostoById(this.postos[0].cod_tipo).subscribe((dtTipo:any)=>{
						this.tipoPosto = dtTipo.result;
						console.log("Tipo do posto ==== " , this.tipoPosto);
